defmodule TeslaMate.FCM.Pusher do
  use GenServer
  require Logger

  alias TeslaMate.{Repo, FCM.TokenStore, Notifications.Settings}
  ...
  def init(_opts) do
    ...
    for car_id <- car_ids do
      :ok = Phoenix.PubSub.subscribe(TeslaMate.PubSub, "TeslaMate.Vehicles.Vehicle/summary/#{car_id}")
    end
  ...
  defp send_to_all(tokens, event, title, body, summary, access_token) do
    ...
    for token <- tokens do
      ...
      case Finch.request(Finch.build(:post, url, headers, payload), TeslaMate.HTTP) do
        {:ok, %{status: 200}} ->
  ...
  defp exchange_for_token(jwt) do
    ...
    case Finch.request(req, TeslaMate.HTTP) do
