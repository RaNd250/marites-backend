defmodule Marites.Import do
  ...
  alias Marites.{Vehicles, Repair, Log}
  alias Marites.Log.{Car, State}
  ...
  def subscribe, do: Phoenix.PubSub.subscribe(Marites.PubSub, @topic)
  ...
  def handle_event(:internal, :broadcast, state, data) do
    :ok = Phoenix.PubSub.broadcast(Marites.PubSub, @topic, Status.into(state, data))
    :keep_state_and_data
  end
