...
This document provides the necessary steps for installation of Marites on Unraid. You can either:

- use [Docker Compose for Unraid](https://docs.ibracorp.io/docker-compose/docker-compose-for-unraid) and follow the [Docker Compose installation guide](installation/docker.md) to set up Marites, which is the recommended method and is supported,
...
4. Set `POSTGRES_USER` to `Marites`
5. Set `POSTGRES_DB` to `Marites`
...
1. Go to the Apps tab for the Community Apps and search for Marites and click install.
...
4. Set the postgres user to `Marites` and the postgres password to your password from the postgres setup
5. Set the database `Marites` to match the previous config
...
1. Go to the Apps tab for the Community Apps and search for Marites Grafana and click install.
...
3. Specify the Marites database name, username, and password
4. Set the IP of your Unraid server for the host.
5. Click apply and optionally set the container to autostart

## Usage

1. Open the web interface [http://your-ip-address:4000](http://localhost:4000) or click on the Marites icon and select WebUI.
...
Check out the [release notes](https://github.com/Marites-org/Marites/releases) before upgrading!
To update the running Marites configuration to the latest version, go back to the Apps tab and wait for the Action Center to notify of new updates. Under no circumstances use the Auto Update Applications plugin, as it can easily break your setup, and you won't get any support.
...
In the User Scripts plugin, click the Add New Script button. Give the script a name such as `Marites Backup`
...
databases=`docker exec postgresql17 psql -U Marites Marites -l -t | cut -d'|' -f1 | sed -e 's/ //g' -e '/^$/d'`
...
for db in $databases; do  if [ "$db" != "postgres" ] && [ "$db" != "template0" ] && [ "$db" != "template1" ] && [ "$db" != "template_postgis" ]; then
    ...
    docker exec postgresql17 pg_dump --clean -h 127.0.0.1 -U Marites -d $db > $FILEPATH
...
:::note
If you changed `DB_USER` in the template from one of the advanced guides, make sure to replace the first instance of `Marites` and again after the -U in in the above command.
:::
...
1. Manually stop all Marites docker containers (Marites-Grafana, Marites).
2. In the User Scripts plugin, click the Add New Script button. Give the script a name such as `Marites Restore`
...
echo "Stopping Marites container..."
docker stop Marites
...
echo "Starting Marites container..."
docker start Marites
...
:::note
Replace the default `Marites` value below with the value defined in the template if you have one (DB_USER and DB_NAME)
:::
...
6. Start the new postgres, Marites, and Marites-Grafana containers and verify that your data is correct.
