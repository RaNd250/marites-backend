...
- Both publicly accessible services, Marites and Grafana, sit behind a reverse proxy (Traefik) which terminates HTTPS traffic
...
> Please note that this is only **an example** of how Marites can be used in a more advanced scenario. Depending on your use case, you may need to make some adjustments, primarily to the traefik configuration. For more information, see the [traefik docs](https://docs.traefik.io/).

## Requirements

- One public FQDN, for example `Marites.example.com` (substitute your domainname throughout the examples below)
...
services:
  Marites:
    ...
    labels:
      traefik.enable: "true"
      traefik.http.services.Marites.loadbalancer.server.port: "4000"
      traefik.http.middlewares.redirect.redirectscheme.scheme: "https"
      traefik.http.middlewares.Marites-auth.basicauth.realm: "Marites"
      traefik.http.middlewares.Marites-auth.basicauth.usersfile: "/auth/.htpasswd"
      traefik.http.routers.Marites-insecure.rule: "Host(`${FQDN_TM}`)"
      traefik.http.routers.Marites-insecure.middlewares: "redirect"
      traefik.http.routers.Marites.rule: "Host(`${FQDN_TM}`)"
      traefik.http.routers.Marites.middlewares: "Marites-auth"
      traefik.http.routers.Marites.entrypoints: "websecure"
      traefik.http.routers.Marites.tls.certresolver: "tmhttpchallenge"
    cap_drop:
      - ALL
...
1. Open the web interface `https://Marites.example.com`
2. Sign in with your Tesla account
3. In the _Settings_ page, update the _URLs_ fields. Set _Web App_ to `https://Marites.example.com` and _Dashboards_ to `https://Marites.example.com/grafana`
